...
ENODE0_ID=$(sudo podman run --rm -dt --cap-add=sys_nice --mac-address="${ENODE0_MAC_ADDR}" --name enode0 localhost/wisca/wiscanet)
...
ENODE1_ID=$(sudo podman run --rm -dt --cap-add=sys_nice --mac-address="${ENODE1_MAC_ADDR}"   --name enode1 localhost/wisca/wiscanet)
